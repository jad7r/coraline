"""
Incident workspace — the deterministic domain layer behind the `coreline` CLI.

This is a FRONT-END helper (ADR-0002 §4), not new backend service code. It owns the two
things ``core`` does not yet implement — incident **state** and an append-only **audit
log** — and wires the mature, tested core for everything crypto-bearing:
