...
- Secrets: HMAC webhook secret loaded from env / OS keychain (see security/secrets.py)
...
from services.jira_webhook_listener.security.secrets import (
    initialize_secrets,
    SecretLoadError,
)
from services.jira_webhook_listener.config import get_settings
...
from services.jira_webhook_listener.routes import webhooks, health
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info("service.loading_secrets", msg="Loading secrets from environment / OS keychain...")
    ...
    # Close Redis connection. Prefer aclose() (redis>=5); fall back to the
    # deprecated close() for older clients.
    if hasattr(app.state, 'redis'):
        closer = getattr(app.state.redis, "aclose", None) or app.state.redis.close
        await closer()
...
if __name__ == "__main__":
    ...
    # Use the import string so uvicorn can reload; the service must be launched
    # from the repository root (e.g. `python -m services.jira_webhook_listener.main`)
    # so that the `services` package is importable.
    uvicorn.run(
        "services.jira_webhook_listener.main:app",
        host=config.host,
        port=config.port,
